...
1. **Maximum Results**: Return at most 10 most relevant file locations
2. **Relevance Scoring**: Prioritize files by:
   - Direct keyword matches (highest)
   - File/directory name matches (high)
   - Related component matches (medium)
   - Indirect references (low)
3. **Summary First**: Start with a brief summary before listing files
4. **Group Similar**: Combine similar files into counts (e.g., "5 test files in /tests/")
5. **Omit Duplicates**: Don't list multiple files with same purpose
